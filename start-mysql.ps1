...
$root = [IO.Path]::GetDirectoryName($base)
$dataLeaf = [IO.Path]::GetFileName($data)
if ($dataLeaf -match '^mysql-(.+)-data$') {
    $errorLog = Join-Path $root "mysql-$($Matches[1])-error.log"
}
else {
    $errorLog = Join-Path $root 'mysql-error.log'
}
...
$process = Start-Process -FilePath $mysqld -ArgumentList $mysqldArgs -WorkingDirectory $root -WindowStyle Hidden -PassThru
