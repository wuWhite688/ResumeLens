...
- **招聘方视角**：匹配引擎本身是双向的——简历 ↔ JD 的证据检索与打分不区分立场，当前产品选择了求职者一侧（一份简历 × 岗位库）。扩展为 HR 侧「一个 JD × 批量简历」的候选人筛选，需要补齐批量简历导入、候选人排行视图与推荐 / 淘汰理由展示，按招聘场景重新设计配额（现为每用户 30 份简历、10 分钟 10 次分析），并处理候选人隐私与筛选公平性问题。
